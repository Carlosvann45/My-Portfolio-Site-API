 ...
 */
const handleTrustedError = (
  err: BadRequest | Unauthorized | NotFound | InternalServerError,
  response: Response,
): void => {
...
const handleCriticalError = (
  err: Error | BadRequest | Unauthorized | NotFound | InternalServerError,
  response?: Response,
): void => {
  if (response) {
    response.status(HttpCode.INTERNAL_SERVER_ERROR).json({
      name: HttpName.INTERNAL_SERVER_ERROR,
      message: Errors.APP_ERROR,
    });
  }
  console.log(err);
 ...
 */
const errorHandler = (
  err: BadRequest | Unauthorized | NotFound | InternalServerError | Error,
  req: Request,
  res: Response,
) => {
  if (isTrustedError(err) && res) {
    handleTrustedError(
      err as BadRequest | Unauthorized | NotFound | InternalServerError,
      res,
    );
  } else {
